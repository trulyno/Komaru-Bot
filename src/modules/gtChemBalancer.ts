...
type EquationState = 'dust' | 'fluid' | 'catalyst';
...
function getTotalAtomCount(formula: string): number {
    const counts = parseFormula(formula);
    return Array.from(counts.values()).reduce((sum, count) => sum + count, 0);
}

function parseEquation(input: string): ParsedEquation {
...
function getUniqueElements(reactants: ParsedMember[], products: ParsedMember[]): string[] {
...
function getEffectiveQuantity(member: ParsedMember, coefficient: number): number {
...
function isBalancedWithCoefficients(
    reactants: ParsedMember[],
    products: ParsedMember[],
    coefficients: number[],
): boolean {
...
function findBalancedCoefficients(
    reactants: ParsedMember[],
    products: ParsedMember[],
): number[] | null {
...
function formatMember(member: ParsedMember, coefficient: number): string {
...
function formatEquation(parsed: ParsedEquation, coefficients: number[]): string {
    const reactantParts = parsed.reactants.map((member, index) =>
        formatMember(member, coefficients[index]),
    );
    const productParts = parsed.products.map((member, index) =>
        formatMember(member, coefficients[parsed.reactants.length + index]),
    );
    return `${reactantParts.join(' + ')} ${parsed.arrow} ${productParts.join(' + ')}`;
}

function formatEquationWithCatalysts(parsed: ParsedEquation, coefficients: number[]): string {
    const allMembers = [...parsed.reactants, ...parsed.products];
