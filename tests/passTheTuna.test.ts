...
import { createPassTheTunaEngine, loadPassTheTunaState, loadPassTheTunaConfig, savePassTheTunaState, savePassTheTunaConfig, type PassTheTunaState } from '../src/passTheTuna';

async function runTests() {
    console.log('--- Starting Pass The Tuna Unit Tests ---');
    ...
    const chain = engine.startChain({ channelId: 'channel-1', guildId: 'guild-1', now: 1_000, config: configOverride });
    ...
    const blockedOutcome = engine.handleAction({ userId: 'user-1', action: 'take', now: 2_000 + 2_000 });
    assert.strictEqual(blockedOutcome.blocked, true);
    assert.strictEqual(blockedOutcome.reason, 'grace_period');

    const sameUserAgain = engine.handleAction({ userId: 'user-1', action: 'pass', now: 2_000 + 10_000 });
    assert.strictEqual(sameUserAgain.blocked, true);
    assert.strictEqual(sameUserAgain.reason, 'same_user');

    const secondPassOutcome = engine.handleAction({ userId: 'user-2', action: 'pass', now: 2_000 + 6_000 });
    assert.strictEqual(secondPassOutcome.chainLength, 2);
    assert.strictEqual(secondPassOutcome.score, 20);

    const takeOutcome = engine.handleAction({ userId: 'user-3', action: 'take', now: 2_000 + 10_000 });
    ...
    const idleChain = engine.startChain({ channelId: 'channel-2', guildId: 'guild-1', now: 20_000, config: idlePenaltyConfig });
    const idleOutcome = engine.handleAction({ userId: 'user-4', action: 'pass', now: 20_000 + 3_600_000 + 1 });
    ...
    console.log('--- ALL PASS THE TUNA TESTS PASSED SUCCESSFULLY! ---');
}
